...
final_epoch = 1;
%% Prep Training Data 
%%% Pass the training data through pretrained rbm_1 to get respective hid_unit_1 activations 
pass = zeros(g_numcases, numhid, g_numbatches);
parfor batch = 1:g_numbatches
    ...
    pass(:,:,batch) = sigmoid_pass;
end

% For computing overfitting / used in early stopping
% "randomize", the validation set used in the computation 
val_data_1 = val_data(1: size(val_data,1)/3,:);
val_data_2 = val_data(1 + size(val_data,1)/3 :size(val_data,1)/3 * 2 ,:);
val_data_3 = val_data(1 + size(val_data,1)/3 * 2: size(val_data,1),:);
% create g_pass for subset of train data:
r = randperm(g_numbatches,floor(size(val_data_1,1)/g_numcases)); %with random interval, size(subtrain)=size(val)
...
g_pass_subtrain = 1./(1 + exp(-g_train_flat*vishid_1 - repmat(hidbiases_1,size(val_data_1,1),1)));  

%% %%%%%%%%%%%%%% TRAIN RBM2 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Load parameters from the illiterate Model: 
g_numdims  = numhid;
vishid     = vishid_2;
hidbiases  = hidbiases_2;
visbiases  = visbiases_2;
...
%%%%%%% START POSITIVE PHASE %%%%%%%%%%%%%%%%
        ...
        data = pass(:,:,batch); % of dim: g_numcases x numhid
        % apply dropout
        r1 = (rand(g_numcases,numhid)<=p_layer1);
        data = data .* r1;
...
%%%%%%% START NEGATIVE PHASE  %%%%%%%%%%%%%%%%
        ...
        negdata = negdata .* r1; % apply dropout 
...
%%%%%%% END OF UPDATES %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
    ...
    %pass_val is changed in each "overfitting-computation" // "randomized"
    v_perm = randi(3);
    if ~no_N_img
        if v_perm==1
            pass_val = 1./(1 + exp(-val_data_1*vishid_1 - repmat(hidbiases_1,size(val_data_1,1),1)));  
        elseif v_perm == 2
            pass_val = 1./(1 + exp(-val_data_2*vishid_1 - repmat(hidbiases_1,size(val_data_2,1),1)));  
        elseif v_perm == 3
            pass_val = 1./(1 + exp(-val_data_3*vishid_1 - repmat(hidbiases_1,size(val_data_3,1),1)));  
        end
    else
        pass_val = (v_perm==1) * val_data_1 + (v_perm==2) * val_data_2 + (v_perm==3) * val_data_3;
    end
    % compute overfitting measures:

    %%% @todo??? add the multiplication, i.e. "scaling inference", i.e. ( ) .* p
    ...
    overfitting_g_2(epoch,2) = mean(-(pass_val*visbiases' + sum(log( 1+exp(pass_val*vishid + repmat(hidbiases,size(pass_val,1),1))),2)));
