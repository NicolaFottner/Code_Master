...
if numhid3 == 0
    rbm2_pass_test = hid_out_2(1:index, :); % basically like the test set from before
    str_hid3 = "";
else
    rbm2_pass_test = hid_out_3(1:index, :);
    str_hid3 = "three/";
end
...
if geo_shape_class == 6
    ...
    r1 = ones(size(test_t,1)/6,1);r2 = ones(size(test_t,1)/6,1)*2;r3 = ones(size(test_t,1)/6,1)*3;
    r4 = ones(size(test_t,1)/6,1)*4;r5 = ones(size(test_t,1)/6,1)*5;r6 = ones(size(test_t,1)/6,1)*6;
    ...
    r1 = ones(size(test_t,1)/12,1);r2 = ones(size(test_t,1)/12,1)*2;r3 = ones(size(test_t,1)/12,1)*3;
    r4 = ones(size(test_t,1)/12,1)*4;r5 = ones(size(test_t,1)/12,1)*5;r6 = ones(size(test_t,1)/12,1)*6;
    r7 = ones(size(test_t,1)/12,1)*7;r8 = ones(size(test_t,1)/12,1)*8;r9 = ones(size(test_t,1)/12,1)*9;
    r10 = ones(size(test_t,1)/12,1)*10;r11 = ones(size(test_t,1)/12,1)*11;r12 = ones(size(test_t,1)/12,1)*12;
    acc1 = (max_1 == r1);acc2 = (max_2 == r2);acc3 = (max_3 == r3);
    acc4 = (max_4 == r4);acc5 = (max_5 == r5);acc6 = (max_6 == r6);
